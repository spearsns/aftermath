<?php
    ...
    $campaignName = parse_url($url, PHP_URL_PATH);
    $txtFileName = parse_url($url, PHP_URL_PATH) . '.txt';

        $query = mysqli_query($conn, "SELECT * FROM campaigns WHERE player = '$username';");
        if(mysqli_num_rows($query) > 0){
         $query  = mysqli_query($conn, "UPDATE campaigns
          SET player_status = '1'
          WHERE player = '$username';");
        } else {
          $query = mysqli_query($conn, "INSERT INTO campaigns (campaignName, txtFile, player, player_status, charName)
          VALUES ('$campaignName', '$txtFileName', '$username', '1', '$charName');");
} 
?>
...
<div class="row ceiling">
  <div class="large-6 columns">
    <?php $sql="SELECT campaignName FROM campaigns WHERE player_status = '1' AND player = '$username';";
  ...
  <div class="large-6 columns hotbox">
    <?php
      if (isset($_SESSION['id']) == true){
        $url = "http://".$_SERVER['HTTP_HOST'].$_SERVER['REQUEST_URI'];
        $charName = parse_url($url, PHP_URL_QUERY);
      echo "<p class='error'>Character Used:  " .$charName ." </p>";
    } else {
      echo "<p class='error'>Please sign in to continue</p>";
    }
    ?>
